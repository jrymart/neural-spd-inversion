...
class SineTopographyDataset(Dataset):
    ...
    def __init__(self, width, height, orientation='y', valleys = np.arange(3,100),norm_to_zero=True, transform=None):
        """
        Initialize the dataset with a sine wave topography.
        :param width: Width of the topography.
        :param height: Height of the topography.
        :param orientation: Orientation of the sine wave ('x' or 'y').
        :param valleys: Number of valleys in the sine wave.
        """
        self.width = width
        self.height = height
    ...
    def __getitem__(self, idx):
        period =self.valleys[idx] * 2 * np.pi
        x, y = np.meshgrid(np.linspace(0, np.pi, self.width),
            #np.linspace(np.pi/2, period + np.pi/2, self.width),
                           np.linspace(np.sin(np.pi/2), period + np.pi/2, self.height))
...
class LandlabBatchDataset(Dataset):
    ...
    def __init__(self, db_path, dataset_dir, label_query, filter_query="", trim=5, normalize=True, inputs_mean=None,
                 inputs_std=None, labels_mean=None, labels_std=None, transform=None):
        """
        Initialize the dataset with a path to the SQLite database.
        
        Args:
            transform: Optional transform to apply to the data (e.g., shuffling augmentations)
        """
...
def build_datasets_from_db(db_path, dataset_dir, label_query, filter_query="", split_by="model_param.seed",
                           ...
                           labels_mean=None, labels_std=None, train_transform=None, test_transform=None, **kwargs):
    """
