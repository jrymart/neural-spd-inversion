...
if not RECALCULATE_STATS:
    ...
    statistics = {}
datasets = TOPOGRAPHIC_DERIVATIVES + [MODEL_DEM_DIR]
for noise in NOISE_LEVELS:
    noise = str(noise).replace('.', '_')
    if noise not in statistics:
        statistics[noise] = {}
    noise_stats = statistics[noise]
    for derivative, derivative_f in TOPOGRAPHIC_DERIVATIVES:
        if derivative not in statistics:
            dataset_path = DATA_PATH / noise / deriviative
            dataset_paths = [dataset_path / f"{name}.npy" for name in train_run_ids]
            noise_stats[derivative] = get_array_statistics(dataset_paths,MODEL_ARRAY_CROP)
