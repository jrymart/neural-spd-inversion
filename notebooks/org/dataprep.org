...
from config import FLOW_METHOD, MODEL_ACC_PATH, MODEL_RESOLUTION, REPROCESS_DATA

for dem_path in model_dem_paths:
    dem_id = dem_path.stem
    slope_path = DATA_PATH / MODEL_SLOPE_PATH / f"{dem_id}.npy"
    curvature_path = DATA_PATH / MODEL_CURV_PATH / f"{dem_id}.npy"
    acc_path = DATA_PATH / MODEL_ACC_PATH / f"{dem_id}.npy"
    ...
    if not (slope_path.exists() or curvature_path.exists()) or REPROCESS_DATA:
        dz_dy, dz_dx = np.gradient(dem, MODEL_RESOLUTION)
...
# TODO don't need need to join, can import path dir
train_paths = [os.path.join(MODEL_DEM_PATH, f"{name}.npy") for name in train_run_ids]
acc_paths = [os.path.join(DATA_PATH, MODEL_ACC_PATH, f"{name}.npy") for name in train_run_ids]
slope_paths = [os.path.join(DATA_PATH, MODEL_SLOPE_PATH, f"{name}.npy") for name in train_run_ids]
curvature_paths = [os.path.join(DATA_PATH, MODEL_CURV_PATH , f"{name}.npy") for name in train_run_ids]
statistics = {}
...
statistics['slope'] = get_array_statistics(train_slope_paths, MODEL_ARRAY_CROP)
statistics['curvature'] = get_array_statistics(train_curvature_paths, MODEL_ARRAY_CROP)
#+end_src
...
['labels'] = {'labels_mean': np.mean(labels),
