...
#+PROPERTY: header-args:python :tangle ../../scripts/05_analysis.py

Here we concatenate performance of the networks as evaluated on the test set as well as valley spacing test, draiange density test, and shuffling tests.  We also calculate bulk performance (primarily RMSE) for the network
#+begin_src python
import os
import pandas as pd
