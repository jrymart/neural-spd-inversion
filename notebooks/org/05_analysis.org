#+title: Analysis
#+PROPERTY: header-args:python :python ../../.venv/bin/python :session analysis
#+PROPERTY: header-args:jupyter-python :tangle ../../scripts/05_analysis.py

Here we concatenate performance of the networks as evaluated on the test set as well as valley spacing test, drainage density test, and shuffling tests. We also calculate bulk performance (primarily RMSE) for the network
...
For a given directory containing results of the experiment, merge all the files, and calculate error
...
Here we process all of our test cases
