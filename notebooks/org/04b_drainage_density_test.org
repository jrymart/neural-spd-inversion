#+title: 04b Drainage Density Test
#+PROPERTY: header-args:python :python ../../.venv/bin/python :session drainagedensity
#+PROPERTY: header-args:jupyter-python :tangle ../../scripts/04b_drainage_density.py
Now we create a dataset of varying drainage density and see how the trained network is able to infer it.  

#+begin_src jypyter-python
import sqlite3
...
The ~AdaptiveThresholdDataset~ takes in an input array, and returns a series of binary masks at evenly spaced thresholds of data.  Here we select the flow accumulation raster for suitably low \(D/K\) run (a.k.a one that is highly dissected) and create what are functionally channel masks with different upstream area thresholds.
#+begin_src jypyter-python
...
We select the weights for the neural networks trained to predict \(\log_{10}(D/K)\) from elevation.
#+begin_src jypyter-python
...
For each set of weights,  load the neural network, and test it against the drainage density dataset.  We calculate an explicit drainage density based on pixel size.
#+begin_src jypyter-python
...
Save the results as csv files for later analysis.
#+begin_src jypyter-python
