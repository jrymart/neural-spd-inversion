...
SEEDS=(0 10 20 30)
NOISE=("0" "0-1")
RESOLUTIONS=("res5m" "res7m" "res8m" "res10m")
...
res_idx=$(( (SLURM_ARRAY_TASK_ID / num_labels) % num_res ))
noise_idx=$(( (SLURM_ARRAY_TASK_ID / (num_labels * num_res)) % num_noise ))
seed_idx=$(( SLURM_ARRAY_TASK_ID / (num_labels * num_res * num_noise) ))
...
echo "Copying /projects/joma0457/neural-spd-inversion/data/$SELECTED_RES/$SELECTED_NOISE/elevation to $TASK_DATA"
start_copy=$(date +%s)
cp -r "/projects/joma0457/neural-spd-inversion/data/$SELECTED_RES/$SELECTED_NOISE/elevation" "$TASK_DATA/"
