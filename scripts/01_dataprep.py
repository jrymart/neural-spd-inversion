...
def resample_dem(dem_array, current_resolution, target_resolution):
    """Resample DEM to different resolution using bilinear interpolation."""
    scale_factor = current_resolution / target_resolution
    resampled = zoom(dem_array, scale_factor, order=1)
    return resampled
...
for noise in NOISE_LEVELS:
    ...
    for dem_path in model_dem_paths:
        ...
        dem_path = DATA_PATH / '0' / MODEL_DEM_DIR / dem_path.name
...
# Generate resolution experiment data
from neural_spd.config import RESOLUTION_EXPERIMENTS, get_resolution_experiment_path, MODEL_RESOLUTION

print("\n=== Generating resolution experiment data ===")
for res_name, target_res in RESOLUTION_EXPERIMENTS.items():
    if res_name == 'res5m':
        continue  # Skip baseline, already exists

    print(f"\nProcessing {res_name} ({target_res}m resolution)")

    for noise in NOISE_LEVELS:
        source_dir = DATA_PATH / str(noise).replace('.', '-') / MODEL_DEM_DIR
        target_dir = get_resolution_experiment_path(noise, res_name) / MODEL_DEM_DIR
        target_dir.mkdir(parents=True, exist_ok=True)

        # Resample all DEMs
        for dem_path in source_dir.glob("*.npy"):
            target_path = target_dir / dem_path.name

            if not target_path.exists() or REPROCESS_DATA:
                dem_array = np.load(dem_path)
                resampled = resample_dem(dem_array, MODEL_RESOLUTION, target_res)
                np.save(target_path, resampled)

        print(f"  Resampled {len(list(source_dir.glob('*.npy')))} DEMs for noise={noise}")
...
# Calculate statistics for resolution experiments
from neural_spd.config import get_resolution_value
print("\n=== Calculating statistics for resolution experiments ===")
for res_name in RESOLUTION_EXPERIMENTS.keys():
    for noise in NOISE_LEVELS:
        noise_str = str(noise).replace('.', '-')
        stats_key = f"{res_name}_{noise_str}_elevation"

        if stats_key not in statistics or RECALCULATE_STATS:
            data_dir = get_resolution_experiment_path(noise, res_name) / MODEL_DEM_DIR
            data_paths = [data_dir / f"{name}.npy" for name in train_run_ids]

            # Scale crop proportionally to resolution
            resolution = get_resolution_value(res_name)
            crop = int(MODEL_ARRAY_CROP * MODEL_RESOLUTION / resolution)

            if __name__ == '__main__' and num_cpus > 1:
                statistics[stats_key] = get_parallel_statistics(data_paths, crop, num_cpus)
            else:
                statistics[stats_key] = get_array_statistics(data_paths, crop)

            print(f"  {stats_key}: mean={statistics[stats_key]['inputs_mean']:.4f}, std={statistics[stats_key]['inputs_std']:.4f}")
